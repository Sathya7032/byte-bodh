  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (tasksRes.data?.success && Array.isArray(tasksRes.data.data)) {
        ...
        totalTasksCount = tasksRes.data.data.length;
        completedTasksCount = tasksRes.data.data.filter(t => t.status === "COMPLETED").length;
      }
      ...
      let activeTemplateName = "None";
      let totalTemplatesOwned = 0;
      const userTemplates = userTemplatesRes.data?.success
        ? (userTemplatesRes.data.data || [])
        : (Array.isArray(userTemplatesRes.data) ? userTemplatesRes.data : []);
      const allTemplates = allTemplatesRes.data?.success
        ? (allTemplatesRes.data.data || [])
        : (Array.isArray(allTemplatesRes.data) ? allTemplatesRes.data : []);
      totalTemplatesOwned = userTemplates.length;
      const activeUserTemplate = userTemplates.find(ut => ut.active === true);
      if (activeUserTemplate) {
        // Look up the template name from the full list
        const matchedTemplate = allTemplates.find(t => t.id === activeUserTemplate.templateId);
        activeTemplateName = matchedTemplate?.templateName || activeUserTemplate.templateName || "Active";
      }
  ...
  return (
    <DashboardLayout containerClassName="w-full space-y-8 flex flex-col bg-transparent animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
        ...
        {/* CARD 2: SETUP PROGRESS + TASKS */}
        <div className="group relative bg-white border border-slate-200 rounded-3xl p-5 hover:shadow-xl hover:border-amber-200 transition-all duration-300 overflow-hidden shadow-sm flex flex-col justify-between">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 space-y-2">
              {recentTasks.length === 0 ? (
                <p className="text-[10px] text-slate-400 font-semibold text-center py-1">No tasks yet — add milestones to track progress</p>
              ) : (
                recentTasks.slice(0, 2).map((task) => {
                  const isCompleted = task.status === "COMPLETED";
                  const isOverdue = task.dueDate && new Date(task.dueDate) < new Date() && !isCompleted;
                  return (
                    <div key={task.id} className="flex items-center gap-2 p-1.5 rounded-xl bg-slate-50/70 border border-slate-100/80">
                      <span className={`w-5 h-5 rounded-md flex items-center justify-center text-[9px] font-black flex-shrink-0 border ${isCompleted ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-amber-50 border-amber-100 text-amber-500"
                        }`}>
                        {isCompleted ? "✓" : "⏳"}
                      </span>
                      <span className={`text-[10px] font-bold truncate flex-1 ${isCompleted ? "text-slate-400 line-through" : "text-slate-700"}`}>
                        {task.title}
                      </span>
                      {isOverdue && (
                        <span className="text-[8px] bg-rose-50 border border-rose-100 text-rose-500 font-black px-1 rounded flex-shrink-0">LATE</span>
                      )}
                    </div>
                  );
                })
          ...
          <Link to="/tasks" className="text-[10px] text-amber-600 font-extrabold hover:underline flex items-center gap-1 mt-3 relative z-10">
            View All Tasks <FaArrowRight size={8} />
          </Link>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between min-h-[200px]">
          <div>
            ...
            {recentTasks.length === 0 ? (
              <div className="text-center py-8 flex flex-col items-center justify-center">
                <span className="text-xl mb-1">📅</span>
                <p className="text-xs text-slate-400 font-bold">No active tasks found</p>
                <p className="text-[10px] text-slate-400 mt-0.5 font-semibold">Add milestones to track your goals and show progress.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {recentTasks.map((task) => {
                  ...
                  return (
                    <div key={task.id} className="flex items-start gap-3 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black border ${isCompleted
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex justify-between items-baseline gap-1.5">
                          <h4 className={`text-[11px] font-bold truncate ${isCompleted ? "text-slate-400 line-through" : "text-slate-800"}`}>
                            {task.title}
                          </h4>
                        </div>
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {isOverdue && (
                            <span className="text-[8px] bg-rose-50 border border-rose-100 text-rose-650 font-black px-1.5 py-0.2 rounded">
                              OVERDUE
                            </span>
                          )}
                        </div>
